...
public static class InitCommand
{
    ...
    private const string ConfigTemplate = """
        {
          ...
          // variable, for example SLIMSHADY_ModelName or the nested SLIMSHADY_SecondOpinion__ModelName.

          // Git remote to clone and review
          "repositoryUrl": "https://github.com/your-org/your-repo.git",
          "branch": "develop",

          // Absolute path of the working tree SlimShady owns; it is OVERWRITTEN DESTRUCTIVELY on every run.
          ...
          "stateFilePath": "slimshady.state.json",
          ...
          // Example: { "endpoint": "https://api.openai.com/v1", "modelName": "gpt-5", "apiKey": "env:SLIMSHADY_SECOND_OPINION_KEY", "contextLines": 30, "maxFileReads": 5, "reviewSkippedFiles": true }
          "secondOpinion": null,
          ...
          //                  "password": "env:SLIMSHADY_SMTP_PASSWORD", "sendOn": "high", "attachReports": true }
          // ACS example:   { "provider": "azureCommunicationServices", "from": "DoNotReply@your-verified-domain.com",
          //                  "to": ["dev@you.com"], "acsConnectionString": "env:SLIMSHADY_ACS_CONNECTION", "sendOn": "high" }
          "email": null,

          "logLevel": "Information",
          "logFilePath": "logs/slimshady-.log",
    ...
    /// <summary>Creates slimshady.json and review-prompt.txt in <paramref name="directory"/>; refuses to overwrite existing files</summary>
    public static int Run(string directory)
    {
        string configPath = Path.Combine(directory, SlimShadyConfig.FileName);
        string promptPath = Path.Combine(directory, PromptFileName);

        if (File.Exists(configPath) || File.Exists(promptPath))
        {
            Console.Error.WriteLine($"Refusing to overwrite an existing {SlimShadyConfig.FileName} or {PromptFileName} in {directory}");
            return 1;
        }
        ...
        Console.WriteLine($"Wrote {SlimShadyConfig.FileName} and {PromptFileName} to {directory}");
        Console.WriteLine("Edit the config (repository, branch, working tree, git path, model endpoint and name), then run 'SlimShady verify' to prove tool-calling before the first review run. See DOCUMENTATION.md for every option");
