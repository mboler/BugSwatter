...
/// <summary>Configuration for a SlimShady run, loaded from slimshady.json in the current working directory. The current directory only selects which config to load; all destructive operations target the absolute working-tree path declared inside it</summary>
public sealed class SlimShadyConfig
{
    /// <summary>Name of the config file expected in the current working directory</summary>
    public const string FileName = "slimshady.json";
    ...
    /// <summary>Absolute path of the working tree SlimShady owns and refreshes destructively on every run</summary>
    ...
    public string StateFilePath { get; init; } = "slimshady.state.json";
    ...
    public string LogFilePath { get; init; } = "logs/slimshady-.log";
    ...
    public static SlimShadyConfig Load(string directory) => LoadFile(Path.Combine(directory, FileName));

    /// <summary>Loads and validates the configuration from an explicit file path</summary>
    public static SlimShadyConfig LoadFile(string path)
    {
        if (!File.Exists(path))
        {
            throw new SlimShadyFatalException($"Config file not found: {path}. Run 'SlimShady init' to create a starter config");
        }

        SlimShadyConfig config;
        try
        {
            config = ConfigLoader.Load<SlimShadyConfig>(path, "SLIMSHADY_");
        }
        catch (Exception ex) when (ex is not SlimShadyFatalException)
        {
            // catch-all: a malformed JSON file or a value that cannot bind to the config shape surfaces as a fatal config error
            throw new SlimShadyFatalException($"Config file {path} could not be read: {ex.Message}", ex);
        }
    ...
    private string ResolveBasePrompt()
    {
        ...
        if (!string.IsNullOrWhiteSpace(ReviewPromptFile))
        {
            ...
            {
                throw new SlimShadyFatalException($"Review prompt file not found: {promptPath}");
            }
    ...
    private void Validate()
    {
        ...
        {
            throw new SlimShadyFatalException($"workingTreePath must be an absolute path, got '{WorkingTreePath}'. SlimShady only operates destructively on an explicitly configured absolute tree, never on the current directory");
        }

        if (!File.Exists(GitExecutablePath))
        {
            throw new SlimShadyFatalException($"gitExecutablePath does not exist: {GitExecutablePath}");
        }
        ...
        {
            throw new SlimShadyFatalException($"modelEndpoint must be an absolute http or https URL, got '{ModelEndpoint}'");
        }
        ...
        {
            throw new SlimShadyFatalException($"perFileRetryCount must be zero or greater, got {PerFileRetryCount}");
        }
        ...
        {
            throw new SlimShadyFatalException("email is configured but secondOpinion is not; email sends only after a completed second opinion, so configure secondOpinion or remove the email block");
        }
    }
    ...
    private static void RequireValue(string value, string fieldName)
    {
        ...
        {
            throw new SlimShadyFatalException($"Config field '{fieldName}' is required and must not be empty");
        }
    }
    ...
    private static void RequirePositive(int value, string fieldName)
    {
        if (value <= 0)
        {
            throw new SlimShadyFatalException($"Config field '{fieldName}' must be greater than zero, got {value}");
        }
    }
}
