...
public sealed class EmailTests : IDisposable
{
    ...
    public void ValidBlockLoads()
    {
        ...
        EmailConfig email = SlimShadyConfig.Load(_directory.Path).Email!;
    ...
    {
        WriteConfigWithEmail(new Dictionary<string, object?> { ["provider"] = "azureCommunicationServices", ["from"] = "DoNotReply@d.com", ["to"] = new[] { "c@d.com" }, ["acsConnectionString"] = "env:SLIMSHADY_ACS_TEST" });
        EmailConfig email = SlimShadyConfig.Load(_directory.Path).Email!;
    ...
    {
        WriteConfigWithEmail(new Dictionary<string, object?> { ["provider"] = "azureCommunicationServices", ["from"] = "DoNotReply@d.com", ["to"] = new[] { "c@d.com" }, ["acsConnectionString"] = "env:SLIMSHADY_ACS_TEST" });
        Assert.NotNull(SlimShadyConfig.Load(_directory.Path).Email);
    }

    [Fact]
    public void AcsConnectionStringResolvesFromEnvironment()
    {
        var config = new EmailConfig { Provider = EmailProvider.AzureCommunicationServices, AcsConnectionString = "env:SLIMSHADY_ACS_RESOLVE_TEST" };

        Environment.SetEnvironmentVariable("SLIMSHADY_ACS_RESOLVE_TEST", "endpoint=https://x;accesskey=secret");
        try
        {
        ...
        {
            Environment.SetEnvironmentVariable("SLIMSHADY_ACS_RESOLVE_TEST", null);
        }
    ...
    {
        var config = new EmailConfig { Username = "user", Password = "env:SLIMSHADY_SMTP_TEST" };

        Environment.SetEnvironmentVariable("SLIMSHADY_SMTP_TEST", "smtp-secret");
        try
        {
        ...
        {
            Environment.SetEnvironmentVariable("SLIMSHADY_SMTP_TEST", null);
        }
