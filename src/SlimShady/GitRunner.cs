...
/// <summary>Runs the configured git executable and captures its output. SlimShady deliberately shells out instead of taking a git library dependency</summary>
public sealed class GitRunner
{
    ...
    public async Task<GitResult> RunAsync(params string[] arguments)
    {
        var startInfo = new ProcessStartInfo
        {
            ...
            // acting on whatever directory SlimShady happened to be launched from
        ...
        using var process = Process.Start(startInfo) ?? throw new SlimShadyFatalException($"Failed to start git at {_gitPath}");
        ...
        catch (OperationCanceledException)
        {
            ...
            throw new SlimShadyFatalException($"git {string.Join(' ', arguments)} did not finish within {GitTimeout.TotalMinutes:0} minutes and was killed");
        }
    ...
    public async Task<string> RunCheckedAsync(params string[] arguments)
    {
        ...
        return result.ExitCode != 0 ? throw new SlimShadyFatalException($"git {string.Join(' ', arguments)} failed with exit code {result.ExitCode}: {Summarize(result.StandardError)}") : result.StandardOutput;
    }
