...
/// <summary>Owns Informant's working tree: first use clones the remote branch and claims the directory with a marker file, every later run verifies the marker and refreshes the tree destructively to a pristine mirror of the remote</summary>
public sealed class WorkingTreeManager
{
    ...
    public async Task EnsureFreshTreeAsync()
    {
        ...
        {
            throw new InformantFatalException($"Refusing to touch {_treePath}: the directory is not empty and has no {MarkerFileName} marker. Informant only runs destructive git operations on a tree it created and claimed itself. Point workingTreePath at a directory dedicated to Informant");
        }
    ...
    private async Task InitializeAsync()
    {
        ...
        string markerText =
            $"This directory is owned by Informant and is overwritten destructively on every run.{Environment.NewLine}Repository: {_repositoryUrl}{Environment.NewLine}Branch: {_branch}{Environment.NewLine}Claimed: {DateTimeOffset.Now:O}{Environment.NewLine}";
    ...
    private async Task RefreshAsync()
    {
        ...
        if (!File.Exists(Path.Combine(_treePath, MarkerFileName)))
        {
            throw new InformantFatalException($"Refusing to run destructive git operations in {_treePath}: the {MarkerFileName} ownership marker is missing");
        }
