...
public sealed record SecondOpinionConfig
{
    ...
    public string ResolvePrompt()
    {
        ...
        if (!string.IsNullOrWhiteSpace(PromptFile))
        {
            ...
            {
                throw new SlimShadyFatalException($"Second-opinion prompt file not found: {promptPath}");
            }
    ...
    internal void Validate()
    {
        ...
        {
            throw new SlimShadyFatalException($"secondOpinion.endpoint must be an absolute http or https URL, got '{Endpoint}'");
        }

        if (string.IsNullOrWhiteSpace(ModelName))
        {
            throw new SlimShadyFatalException("secondOpinion.modelName is required");
        }

        if (RequiresApiKey && !SecretReference.IsReference(ApiKey))
        {
            throw new SlimShadyFatalException("secondOpinion.apiKey must be an env:VARIABLE_NAME or file:PATH reference; API keys are never stored in the config file. Omit the field for local endpoints that need no authentication");
        }

        if (RequestTimeoutSeconds <= 0)
        {
            throw new SlimShadyFatalException($"secondOpinion.requestTimeoutSeconds must be greater than zero, got {RequestTimeoutSeconds}");
        }

        if (ContextLines <= 0)
        {
            throw new SlimShadyFatalException($"secondOpinion.contextLines must be greater than zero, got {ContextLines}");
        }

        if (MaxFileReads < 0)
        {
            throw new SlimShadyFatalException($"secondOpinion.maxFileReads cannot be negative, got {MaxFileReads}");
        }
    }
}
