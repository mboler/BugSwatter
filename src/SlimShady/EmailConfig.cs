...
public sealed record EmailConfig
{
    ...
    internal void Validate()
    {
        ...
        {
            throw new SlimShadyFatalException("email.from is required when the email block is present");
        }

        if (To.Count == 0 || To.Any(string.IsNullOrWhiteSpace))
        {
            throw new SlimShadyFatalException("email.to must list at least one non-empty recipient");
        }
        ...
        switch (Provider)
        {
            ...
            default:
                throw new SlimShadyFatalException($"email.provider '{Provider}' is not supported");
        }
    }
    ...
    private void ValidateSmtp()
    {
        ...
        {
            throw new SlimShadyFatalException("email.smtpHost is required for the smtp provider");
        }

        if (SmtpPort is < 1 or > 65535)
        {
            throw new SlimShadyFatalException($"email.smtpPort must be between 1 and 65535, got {SmtpPort}");
        }

        if (!string.IsNullOrWhiteSpace(Password) && !SecretReference.IsReference(Password))
        {
            throw new SlimShadyFatalException("email.password must be an env:VARIABLE_NAME or file:PATH reference; SMTP passwords are never stored in the config file");
        }
    }
    ...
    private void ValidateAcs()
    {
        ...
        {
            throw new SlimShadyFatalException("email.acsConnectionString is required for the azureCommunicationServices provider and must be an env:VARIABLE_NAME or file:PATH reference; connection strings are never stored in the config file");
        }
    }
}
