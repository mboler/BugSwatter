...
public static class ToolCallingVerifier
{
    ...
    public static async Task<VerificationResult> VerifyAsync(ModelClient client, int maxContextCharacters, CancellationToken cancellationToken = default)
    {
        ...
        string probeDirectory = Path.Combine(Path.GetTempPath(), "slimshady-verify-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(probeDirectory);

        try
        {
            File.WriteAllLines(Path.Combine(probeDirectory, ProbeFileName), ["This file verifies SlimShady tool-calling.", $"The verification token is {ProbeToken}", "End of probe file."]);
    ...
    public static async Task RequireToolCallingAsync(ModelClient client, int maxContextCharacters, CancellationToken cancellationToken = default)
    {
        ...
        if (!result.Success)
        {
            throw new SlimShadyFatalException($"Tool-calling verification failed: {result.Detail}. Tool-calling is a hard requirement and there is no text-only fallback. Check that the endpoint is running, the model name is correct, and the model supports function calling");
        }
