...
public sealed class SlimShadyConfigTests : IDisposable
{
    ...
    {
        WriteConfig();
        var config = SlimShadyConfig.Load(_directory.Path);
        ...
        Assert.Equal("slimshady.state.json", config.StateFilePath);
    ...
    public void LoadFileAcceptsAnExplicitPathWithAnyName()
    {
        ...
        File.Move(Path.Combine(_directory.Path, SlimShadyConfig.FileName), customPath);

        SlimShadyConfig config = SlimShadyConfig.LoadFile(customPath);
        Assert.Equal("https://example.test/repo.git", config.RepositoryUrl);
    }
    ...
    public void ParsesFullModeCaseInsensitively()
    {
        ...
        Assert.Equal(ReviewMode.Full, SlimShadyConfig.Load(_directory.Path).ReviewMode);
    }
    ...
    {
        File.WriteAllText(Path.Combine(_directory.Path, SlimShadyConfig.FileName), "{ not json");
        SlimShadyFatalException ex = Assert.Throws<SlimShadyFatalException>(() => SlimShadyConfig.Load(_directory.Path));
    ...
    {
        WriteConfig();
        string path = Path.Combine(_directory.Path, SlimShadyConfig.FileName);
        string json = File.ReadAllText(path);
        File.WriteAllText(path, "// leading comment\n" + json.TrimEnd().TrimEnd('}') + ",\n// inline comment\n\"logLevel\": \"Debug\",\n}\n");
        Assert.Equal("Debug", SlimShadyConfig.Load(_directory.Path).LogLevel);
    }
    ...
    public void InlinePromptWinsOverFileAndDefault()
    {
        ...
        Assert.Equal("inline prompt", SlimShadyConfig.Load(_directory.Path).ResolveReviewPrompt(EmptyTree()));
    }
    ...
    public void PromptFileIsReadWhenNoInlinePrompt()
    {
        ...
        Assert.Equal("prompt from file", SlimShadyConfig.Load(_directory.Path).ResolveReviewPrompt(EmptyTree()));
    }
    ...
    public void MissingPromptFileThrowsFatal()
    {
        ...
        var config = SlimShadyConfig.Load(_directory.Path);
        Assert.Throws<SlimShadyFatalException>(() => config.ResolveReviewPrompt(EmptyTree()));
    }
    ...
    {
        WriteConfig();
        Assert.Equal(DefaultReviewPrompt.Text, SlimShadyConfig.Load(_directory.Path).ResolveReviewPrompt(EmptyTree()));
    }
    ...
    public void AgentsFileInTreeIsAppendedWhenListed()
    {
        ...
        string prompt = SlimShadyConfig.Load(_directory.Path).ResolveReviewPrompt(tree);
    ...
    public void AbsolutePathIncludeIsAppended()
    {
        ...
        Assert.Contains("absolute include content", SlimShadyConfig.Load(_directory.Path).ResolveReviewPrompt(tree));
    }
    ...
    public void EmptyIncludeListDisablesTheMechanism()
    {
        ...
        Assert.Equal(DefaultReviewPrompt.Text, SlimShadyConfig.Load(_directory.Path).ResolveReviewPrompt(tree));
    }
    ...
    {
        WriteConfig();
        Environment.SetEnvironmentVariable("SLIMSHADY_ModelName", "overridden-model");
        try
        {
            Assert.Equal("overridden-model", SlimShadyConfig.Load(_directory.Path).ModelName);
        }
        finally
        {
            Environment.SetEnvironmentVariable("SLIMSHADY_ModelName", null);
        }
    }
    ...
    public void EnvironmentVariableOverridesANestedConfigValue()
    {
        ...
        Environment.SetEnvironmentVariable("SLIMSHADY_SecondOpinion__ModelName", "env-validator");
        try
        {
            Assert.Equal("env-validator", SlimShadyConfig.Load(_directory.Path).SecondOpinion!.ModelName);
        }
        finally
        {
            Environment.SetEnvironmentVariable("SLIMSHADY_SecondOpinion__ModelName", null);
        }
    }
