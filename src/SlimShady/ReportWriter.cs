...
public sealed class ReportWriter
{
    ...
    public ReportWriter(string directory, string runStamp, string modelName, string modelEndpoint, int maxContextCharacters, int maxFileLines)
    {
        ...
        _path = Path.Combine(directory, $"SlimShady-Report-{runStamp}.md");
    ...
    public void WriteHeader(string repositoryUrl, string branch, ReviewMode mode, string? baselineSha, string tipSha, DateTimeOffset startedAt)
    {
        ...
        builder.AppendLine("# SlimShady Review Report");
        builder.AppendLine();
        ...
        builder.AppendLine();
        builder.AppendLine("Findings are produced by the local review model, which cannot execute code; treat them as leads for human validation. Structure, metadata and skip notes are written deterministically by SlimShady.");
        builder.AppendLine();
        builder.AppendLine("---");
        builder.AppendLine();
