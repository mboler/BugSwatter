...
BugSwatter ships as two executables: the reviewer (`SlimShady`) and the dispatcher (`Marshal`). This document covers both in full. For a short overview see the [README](README.md).
...
- [SlimShady commands](#slimshady-commands)
...
- On first use SlimShady clones the branch and claims the directory with a `.slimshady` marker file. Every later run refuses to touch a non-empty directory without that marker, so pointing the config at a real checkout by accident is caught and stopped before any destructive git command runs.
...
| Command | Effect |
| --- | --- |
| `SlimShady [--config <path>]` | Run a review |
| `SlimShady init` | Write a starter `slimshady.json` and `review-prompt.txt` into the current directory |
| `SlimShady verify [--config <path>]` | Run only the tool-calling verification gate, exit 0 on pass |
| `SlimShady validate [--config <path>]` | Check config, endpoint reachability and secrets without running a review, exit 0 on pass |
| `SlimShady help` | Show usage |

`--config` names the config file explicitly, which removes any dependency on the current working directory; a scheduler or Marshal can invoke `SlimShady --config D:\jobs\my-repo\slimshady.json` from anywhere. Relative paths inside the config (reports, state, logs, prompt file) resolve against the config file's directory. Without `--config`, `slimshady.json` is read from the current directory.

Exit code 0 means success; 1 means a fatal condition that is described on stderr and in the log.

## Configuration

`slimshady.json` is read from the current working directory, or from the file named by `--config`. Comments and trailing commas are allowed. Relative paths resolve against the directory the config was loaded from; the working tree path must be absolute.

| Field | Meaning | Default |
| --- | --- | --- |
| `repositoryUrl` | Git remote to clone and review | required |
| `branch` | Branch to review | required |
| `workingTreePath` | Absolute path of the tree SlimShady owns and destructively refreshes | required |
...
| `stateFilePath` | JSON file holding baseline SHAs keyed by repo and branch | `slimshady.state.json` |
...
| `logFilePath` | Rolling log file path | `logs/slimshady-.log` |
...
- `SLIMSHADY_` for the reviewer, `MARSHAL_` for the dispatcher.
- `SLIMSHADY_ModelName=gpt-x` overrides the top-level `modelName`.
- `SLIMSHADY_SecondOpinion__ModelName=gpt-x` overrides the nested `secondOpinion.modelName`.
...
Alongside the Markdown validated report, the pass writes a machine-readable `SlimShady-Report-<timestamp>-validated.json` (confirmed and discarded findings, per-file severity, and a run-level maximum), which drives the email severity gate and the Marshal dashboard.

## Email

When an `email` block is present, SlimShady emails the reports at the end of a run. Email is gated on a completed Second Opinion, so it never fires for a raw local review, and it is gated on severity so it only interrupts you when it matters. Every decision is recorded as an Email delivery section appended to the validated report, noting whether the mail was sent, skipped or failed, when, the provider, and the recipients.
...
"email": {
  ...
  "username": "bugswatter",
  "password": "env:SLIMSHADY_SMTP_PASSWORD",
  "sendOn": "high",
  "attachReports": true
}
```
...
On Windows, `install` and `remove` talk to the Service Control Manager directly through the API by default; pass `--use-sc` to shell out to sc.exe instead. On Linux they write or remove the systemd unit and need root. `validate` loads the config, checks the SlimShady executable and each job's watch path, resolves webhook secrets, and runs `SlimShady validate` for every job. `--review-all` enqueues every configured repository once at startup and then keeps watching; without it Marshal starts quiet and acts only on triggers.

```jsonc
{
  "slimShadyExecutable": "D:\\deploy\\SlimShady.exe",
  ...
  "jobs": [
    {
      ...
      "slimShadyConfigPath": "D:\\jobs\\bugswatter\\slimshady.json",
      "schedule": ["03:00"],
...
All reviews share the single local model endpoint, so Marshal runs at most one SlimShady review at a time, globally: triggers feed a bounded queue (128 entries; overflow dropped with a warning) consumed by a single serial executor. Duplicates coalesce by repository identity (the job's SlimShady config path, compared case-insensitively on Windows). If a trigger arrives for the repository currently under review, the running review is never killed: a single pending-rerun flag is set, and when the run finishes that repository is enqueued once more. Each child runs as `SlimShady --config <job config>` under supervision; a child exceeding the per-run timeout (default 6 hours) has its whole process tree killed. Marshal reads the report path SlimShady prints on stdout, so it records the exact artifact rather than guessing by timestamp. The queue is in-memory and transient: a restart starts quiet, or re-kick everything with `--review-all`.
