...
//   SLIMSHADY_IT=1
//   SLIMSHADY_IT_ENDPOINT=http://host:port/v1
//   SLIMSHADY_IT_MODEL=your-review-model
// and, to also cover the second-opinion endpoint:
//   SLIMSHADY_IT_SO_ENDPOINT=http://host:port/v1
//   SLIMSHADY_IT_SO_MODEL=your-validator-model
//
// Example (PowerShell), pointing at the review model on one host and the validator on another:
//   $env:SLIMSHADY_IT='1'
//   $env:SLIMSHADY_IT_ENDPOINT='http://192.0.2.57:1234/v1'; $env:SLIMSHADY_IT_MODEL='ornith-1.0-35b'
//   dotnet test tests/SlimShady.Tests
public sealed class LiveIntegrationTests
{
    ...
    {
        Assert.SkipUnless(Live.Enabled, Live.DisabledReason);
        Assert.SkipUnless(Live.SecondOpinionConfigured, "set SLIMSHADY_IT_SO_ENDPOINT and SLIMSHADY_IT_SO_MODEL to cover the second-opinion endpoint");
...
file static class Live
{
    static Live()
    {
        Endpoint = Environment.GetEnvironmentVariable("SLIMSHADY_IT_ENDPOINT") ?? "";
        Model = Environment.GetEnvironmentVariable("SLIMSHADY_IT_MODEL") ?? "";
        SecondOpinionEndpoint = Environment.GetEnvironmentVariable("SLIMSHADY_IT_SO_ENDPOINT") ?? "";
        SecondOpinionModel = Environment.GetEnvironmentVariable("SLIMSHADY_IT_SO_MODEL") ?? "";

        string masterSwitch = Environment.GetEnvironmentVariable("SLIMSHADY_IT") ?? "";
    ...
    public static string DisabledReason => "live integration tests are opt-in: set SLIMSHADY_IT=1 with SLIMSHADY_IT_ENDPOINT and SLIMSHADY_IT_MODEL to run them";
}
