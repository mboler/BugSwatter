...
public sealed class SecretReferenceTests : IDisposable
{
    ...
    {
        Environment.SetEnvironmentVariable("SLIMSHADY_SECRETREF_TEST", "the-secret");
        try
        {
            Assert.Equal("the-secret", SecretReference.Resolve("env:SLIMSHADY_SECRETREF_TEST"));
        }
        finally
        {
            Environment.SetEnvironmentVariable("SLIMSHADY_SECRETREF_TEST", null);
        }
    }
